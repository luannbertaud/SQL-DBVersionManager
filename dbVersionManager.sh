...
printHelp() {
    ...
    res+="     @              @ A prompt will be displayed when bypass is available, be careful.\n"
    ...
    res+="     @--new@Endpoint from which to fetch the last version of database.\n"
    res+="     @              @ A new script will be created with the next available version.\n"
    res+="     @              @ Can be used without argument if provided by .conf.\n"
...
then
    ...
    do
        ...
        if [ "$key" = "db.versionEndpoint" ]; then DbVersionEndpoint=$(echo $val | tr -d '"'); fi
...
while (( "$#" )); do
    case $1 in
        ...
        -y|--yes) PassYes="Yes" ;;
        --new) CreateScript="Yes"; if [[ ! $2 = "" ]]; then DbVersionEndpoint="$2"; fi; shift ;;
...
if [ $CreateScript = "No" ] && [[ "$DBname" = "" || "$DBhost" = "" || "$DBuser" = "" ]];
then
...
exitError() {
    ...
    echo -e " DB version endpoint: $DbVersionEndpoint"
    echo -e "     New script mode: $CreateScript"
    ...
    echo -e "Always yes to prompts: $PassYes"
    echo "=================================/!\================================="
...
#
# New script mode doesn't requires a database connection
#

if [[ "$CreateScript" = "Yes" ]];
then
    if [[ "$DbVersionEndpoint" = "" ]];
    then
      echo "To find the proper version number, you must provide the version endpoint with --new [ENDPOINT]."
      exitClean;
    fi

    currentDBVersion=$(wget -T 3 -q -O - $DbVersionEndpoint)
    if [[ ! $currentDBVersion =~ $versRegx ]];
    then
      echo "Unable to fetch a valid version from [$DbVersionEndpoint] => [$currentDBVersion]."
      exitClean;
    fi
    echo "- Remote database version: $currentDBVersion"

    newDBVersion=$(echo $currentDBVersion | awk -vFS=. -vOFS=. '{$NF++;print}')
    currentLocalDBVersion=${migrationScripts[-1]}
    echo "- Local max version: $currentLocalDBVersion"

    if ( verlte $newDBVersion $currentLocalDBVersion );
    then
      newDBVersion=$(echo $currentLocalDBVersion | awk -vFS=. -vOFS=. '{$NF++;print}')
    fi
    touch $MigrationFolder$newDBVersion
    echo "New script $newDBVersion created for next available version."
    exitClean;
fi
